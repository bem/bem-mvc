...
**В связи с активной разработкой в библиотеках `bem-core` и `bem-components` некоторая функциональность может работать некорректно! Мы работаем над стабилизацией и не откажемся от любой помощи в этом.**
...
Иногда создание моделей с помощью блока `glue` может быть неудобным. Для случая, когда данные для модели генерируются во время шаблонизации, можно использовать блок `model`.
...
В итоге все блоки `model` внутри агрегатора будут объединены в один, и модели будут проинициализированы до инициализации других блоков.
